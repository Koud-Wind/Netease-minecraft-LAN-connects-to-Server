...
public class NeteaseModTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        switch (transformedName) {
            ...
            case "net.minecraft.util.ChatAllowedCharacters":
                return isAllowedCharacterTransform(basicClass);
        }
        return basicClass;
    }
    ...
    private byte[] isAllowedCharacterTransform(byte[] basicClass) {
        ClassReader classReader = new ClassReader(basicClass);
        final ClassWriter classWriter = new ClassWriter(classReader, 2);
        ...
        ClassVisitor classVisitor = new ClassVisitor(ASM4, classWriter) {
            ...
            public MethodVisitor visitMethod(int access, String name, String descriptor, String signature, String[] exceptions) {
                if ((name.equals("isAllowedCharacter") || name.equals("func_71566_a") || name.equals("a")) && descriptor.equals("(C)Z")) {
                    ...
                    methodVisitor.visitMethodInsn(INVOKESTATIC, "com/netease/mc/mod/plugin/NeteaseModTransformer", "isAllowedChatCharacter", "(C)Z", false);
                    methodVisitor.visitInsn(IRETURN);

                    Label label1 = new Label();
                    methodVisitor.visitLabel(label1);

                    methodVisitor.visitLocalVariable("character", "C", null, label0, label1, 0);
    ...
    public static boolean isAllowedChatCharacter(char character) {
        // dont delete
        boolean b = character >= 32 && character != 127;
        return b;
    }
}