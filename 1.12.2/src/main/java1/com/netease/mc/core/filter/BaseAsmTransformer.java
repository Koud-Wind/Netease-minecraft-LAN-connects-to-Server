...
public abstract class BaseAsmTransformer implements IClassTransformer {
    ...
    protected BaseAsmTransformer() {
        ...
        for(Class<?> c : clz.getDeclaredClasses()) {
            if (IMethodTransformer.class.isAssignableFrom(c)) {
                RegisterTransformer annotation = (RegisterTransformer)c.getAnnotation(RegisterTransformer.class);
                if (annotation != null) {
                    Class<? extends IMethodTransformer> clzz = c.asSubclass(IMethodTransformer.class);

                    try {
                        IMethodTransformer transformer = (IMethodTransformer)clzz.newInstance();
                        String srgName = annotation.srgName().equals("") ? annotation.mcpName() : annotation.srgName();
                        this.hookMethod(annotation.className(), srgName, annotation.mcpName(), annotation.desc(), transformer);
    ...
    protected void hookMethod(String className, String srgName, String mcpName, String desc, IMethodTransformer targetTransformer) {
        ...
        ((Map)this.map.get(className)).put(srgName + desc, targetTransformer);
        ((Map)this.map.get(className)).put(mcpName + desc, targetTransformer);
    }
    ...
    public byte[] transform(String obfClassName, String className, byte[] bytes) {
        if (!this.map.containsKey(className)) {
            ...
            Map<String, IMethodTransformer> transMap = (Map)this.map.get(className);
            ...
            for(MethodNode mn : ml) {
                ...
                if (transMap.containsKey(methodName + methodDesc)) {
                    try {
                        ((IMethodTransformer)transMap.get(methodName + methodDesc)).transform(cn, obfClassName, mn, methodName, deobfuscatedEnvironment);
                    } catch (Exception e) {
    ...
    public interface IMethodTransformer {
        void transform(ClassNode var1, String var2, MethodNode var3, String var4, boolean var5);
    }
